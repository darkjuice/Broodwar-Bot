...
int Tools::CountMyUnitsOfType(BWAPI::UnitType type)
{
...
BWAPI::Unit Tools::GetUnitOfType(BWAPI::UnitType type)
{
    ...
    for (auto& unit : BWAPI::Broodwar->self()->getUnits())
    {
        ...
        if (unit->getType() == type && unit->isCompleted())
        {
            if (unit->getType().isWorker() && !unit->isGatheringGas() && !unit->isConstructing()) { return unit; }
...
int Tools:: countNearbyMinerals(BWAPI::Position pos, int radius)
{
    int count = 0;
    for (auto unit : BWAPI::Broodwar->getUnitsInRadius(pos, radius))
    {
...
int Tools::detectConstructing()
{  // count the number os constructing field, does not include add-ons,and static defensive construction 
    const BWAPI::Unitset& myUnits = BWAPI::Broodwar->self()->getUnits();
    int count = 0;
    for (auto& unit : myUnits)
    {
        if (unit->getType().isBuilding() && unit->isBeingConstructed() && !unit->getType().isAddon() && !(unit->getType() == BWAPI::UnitTypes::Terran_Bunker)) {count++;}
    }
    return count;
}
